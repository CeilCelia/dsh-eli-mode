...
- **UI polish (submodule eli-polish)**: adds a KB tab, token stats, tool-call collapsing
  and character art on top of the default theme. **Off by default** — tick "界面润色" in the
...
3. Settings → Plugin Configuration → Eli Mode: edit the persona prompt and the KB
   injection prompt (**takes effect on new sessions**), or tick "界面润色" and swap the
...
| UI polish | Toggle: character art, in-chat "知识库" tab, token stats, tool-call collapsing; applies immediately |
...
- Character art (`ui/art-left.webp` main UI + `ui/art-right.webp` wiki) is a derivative of
  ...
  in [NOTICE](packages/eli-mode/NOTICE):
